...
public class OrderAdapter extends RecyclerView.Adapter<OrderAdapter.MyViewHolder> implements Filterable {
    public OrderAdapter(List<Order> userList, Context context) {
...
String url ;
    ...
    public MyViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View view = LayoutInflater.from(context).inflate(R.layout.upcoming_order_item,viewGroup,false);
...
new UpdateDelivered().execute(user.getKey(),user.getOrderNo(),user.getName(),user.getGST(),user.getAmount(),strDate,Integer.toString(i));
                ...
                Task<Void> uploadref = FirebaseDatabase.getInstance().getReference().child("UpcomingOrders").child(user.getKey()).removeValue();
                userList.remove(i);
            notifyItemRemoved(i);
...
date = itemView.findViewById(R.id.date);
    ...
    public class UpdateDelivered extends AsyncTask<String,Void,Boolean>
    {
        ...
        protected Boolean doInBackground(String... strings) {
            ...
            deliveredref = FirebaseDatabase.getInstance().getReference().child("DeliveredOrders").push();
